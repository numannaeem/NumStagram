...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (loggedUser.following.length > 0) {
      ...
      }).skip(skips).limit(size).sort({ createdAt: -1 }).populate('user').populate('comments.user')
    } else {
...
router.post("/comment/:postId", authMiddleware, async (req, res) => {
  try {
    ...
    await post.save();
    if (post.user.toString() !== userId) {
      await newCommentNotification(postId, userId, newComment._id, post.user.toString(), text)
    }
